  ...
  return (
    <>
      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-4 py-14 sm:px-6 lg:grid-cols-3 lg:px-8">
        {reviews.map((r: any) => (
          <Link key={r.id} to="/reviews/$slug" params={{ slug: r.slug }} className="group flex flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-[var(--shadow-card)] transition-all hover:shadow-[var(--shadow-elegant)]">
            ...
            <div className="flex flex-1 flex-col gap-2 p-5">
              ...
              <p className="mt-auto text-xs text-muted-foreground">{formatDate(r.published_at)}{r.property?.location?.name ? ` · ${r.property.location.name}` : ""}</p>
            </div>
          </Link>
        ))}
      </section>
